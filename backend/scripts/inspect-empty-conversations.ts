/**
 * Diagnostic script — lists ALL conversations with zero messages, regardless
 * of phoneNumber format. Read-only, makes no changes. Use this to understand
 * the actual shape of the "ghost" rows before deciding on a fix/cleanup.
 *
 * USAGE:
 *   npx ts-node scripts/inspect-empty-conversations.ts
 */
...
async function main() {
  ...
  // Breakdown by phoneNumber format
  const lidCount = empties.filter((c) => c.phoneNumber.endsWith('@lid')).length;
  const waCount = empties.filter((c) =>
    c.phoneNumber.endsWith('@s.whatsapp.net'),
  ).length;
